...
class LogzioShipper:
    ...
    def send_to_logzio(self):
        ...
        try:
            ...
            logger.error(
                "Can't establish connection to {0} url. Please make sure your url is a Logz.io valid url. Max retries "
                "of {1} has reached. response: {2}".format(
    ...
    def _is_log_valid_to_be_sent(self, log, log_size):
        if log_size > LogzioShipper.MAX_LOG_SIZE_BYTES:
            logger.error(
                ...
                "{1}".format(LogzioShipper.MAX_LOG_SIZE_BYTES, log))
